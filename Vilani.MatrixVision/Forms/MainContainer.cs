...
namespace Vilani.MatrixVision
{
    public partial class MainContainer : Form
    {
        ...
        public MainContainer()
        {
            ...
            this.camera.BlobDetected += camera_BlobDetected;
            Control.CheckForIllegalCrossThreadCalls = false;
            ...
            if (FeatureSettings.VisionInspectionType == VisionInspectionType.BlobAndSerialPatternMatching
               || FeatureSettings.VisionInspectionType == VisionInspectionType.BlobAnalysis)
            {
                tableLayoutPanel1.SetColumnSpan(panel8, 2);
                pictureBoxPreviousImage.Visible = false;
                panel9.Visible = false;
                tableLayoutPanel1.SetColumnSpan(pictureBoxCapturedImage, 2);
                this.pictureBoxCapturedImage.SizeMode = PictureBoxSizeMode.StretchImage;
                GlobalSettings.BlobConfigSettings = _dbAccess.GetTeachImageConfiguration();
                createPatternImageToolStripMenuItem.Visible = true;
            }

            if (FeatureSettings.IsReportingSupprted)
            {
                reportsToolStripMenuItem.Visible = true;
                reportHistoryToolStripMenuItem.Visible = true;
            }
          
        }
        BlobResult _CurrentBlobResult = null;
        void camera_BlobDetected(BlobResult blobResult)
        {
            _CurrentBlobResult = blobResult;
            this.pictureBoxCapturedImage.Update();
            UpdateCounters(blobResult);
            var isMatching = false;

            isMatching = blobResult.IsValid();
        ...
        private void camera_ShowReport(string modelValue, string plcValue)
        {
            try
            {
                this.BeginInvoke((Action)delegate
                {
                    ...
                    reportForm.ProductName = txtInspectionName.Text;
        ...
        private void cam_PatternMatchDetected(List<PatternResult> matchingResult)
        {
            ...
            try
            {
                ...
                AddResultToReport(matchingResult, isMatching);
                // camera.SetReadyToProcessNextModel();
        ...
        private bool CheckIfResultToInvert(List<PatternResult> matchingResult)
        {
            if (matchingResult.Count > 0)
            {
                var modelID = matchingResult[0].ModelID;
                var model = _dbAccess.GetModelsListDataSource().Where(x => x.ModelID == modelID).FirstOrDefault();
                if (model != null)
                    return model.InvertResult;
            }
            return false;
        }

        private bool ApplyInvertResult(bool isMatching)
        {
            throw new NotImplementedException();
        }

        private void AddResultToReport(List<PatternResult> matchingResult, bool isValid)
        {
        ...
        {
            logger.Info(string.Format("Sending Result {0} ", isMatching));
            if (FeatureSettings.SerialPortCommunicationsSupported)
            {
                try
                {
                    SerialPortVision port = _communicationPort as SerialPortVision;
                    logger.Info(string.Format("Sending On Port Port {0}, Baurd Rate{1} , Parity {2}, Datebits {3}, StopBits {4} ",
                        port.SerialPort.PortName, port.SerialPort.BaudRate, port.SerialPort.Parity, port.SerialPort.DataBits, port.SerialPort.StopBits));
                }
                catch (Exception ex)
                {
            ...
            if (isMatching)
            {
                logger.Info(string.Format("Sending Result Text {0} ", GlobalSettings.OKSendText));
                _communicationPort.Write(GlobalSettings.OKSendText);
            }
            else
            {
                logger.Info(string.Format("Sending Result Text {0} ", GlobalSettings.NotOKSendText));
        ...
        private bool UpdateCounters(List<PatternResult> matchingResult)
        {
            ...
            if (CheckIfResultToInvert(matchingResult))
                flag = !flag;
        ...
        private bool UpdateCounters(BlobResult blobResult)
        {
            bool flag = false;
            flag = blobResult.ListOfInvalidBlobs.Count > 0 ? false : true;
            _inspectionCounters.IncrementCounters(blobResult.ListOfValidBlobs.Count, blobResult.ListOfInvalidBlobs.Count);
            this.txtOKCounts.Text = _inspectionCounters.OKCount.ToString();
            this.txtNotOKCounts.Text = _inspectionCounters.NotOKCount.ToString();
            this.lblTotalCounts.Text = string.Format("Total Counts : {0}", _inspectionCounters.TotalCounts);
            this.lblTotalTriggerRecived.Text = string.Format("Total Triggers : {0}", _inspectionCounters.TotalTriggerRecieved);
                    ...
                    foreach (Item current in referenceImageFiles)
                    {
                        ...
                        if (File.Exists(text))
                        {
                            ...
                            if (GlobalSettings.ShowReactanglesOnSourceImage)
                                ucRefImage.CurrentColor = ReactngleColorCodesContainer.GetInstance.GetElementAt(current.SequenceNumber - 1);
        ...
        //    if (FeatureSettings.VisionInspectionType == VisionInspectionType.BlobAndSerialPatternMatching && _CurrentBlobResult != null)
        //    {
        //        System.Drawing.Rectangle displayRectangle = this.pictureBoxCapturedImage.DisplayRectangle;

        //        foreach (var item in _CurrentBlobResult.ListOfValidBlobs)
        //        {
        //            System.Drawing.Rectangle srcRect = new System.Drawing.Rectangle(item.X, item.Y, item.Width, item.Height);
        ...
        //        foreach (var item in _CurrentBlobResult.ListOfInvalidBlobs)
        //        {
        //            System.Drawing.Rectangle srcRect = new System.Drawing.Rectangle(item.X, item.Y, item.Width, item.Height);
        ...
        //    }
        //    else if (_caputedRectangles != null && _caputedRectangles.Count > 0)
        //    {
        ...
        private void pictureBoxCapturedImage_Paint(object sender, PaintEventArgs e)
        {
            ...
            if ((FeatureSettings.VisionInspectionType == VisionInspectionType.BlobAndSerialPatternMatching
                || FeatureSettings.VisionInspectionType == VisionInspectionType.BlobAnalysis) && _CurrentBlobResult != null)
            {
                ...
                {
                    // var imagePath = Path.Combine(Path.Combine(Application.StartupPath, VisionConstants.CAP_IMG_FOLDER_NAME), item.Key);
                    System.Drawing.Image image = System.Drawing.Image.FromFile(_CurrentCaputuredImagePath);
                    int width = image.Width;
                    int height = image.Height;
                    ...
                    System.Drawing.Rectangle srcRect = new System.Drawing.Rectangle(tarX, tarY, (int)tarWidth, (int)tarHeight);
                    Pen pen;
                    if (Convert.ToBoolean(ConfigurationManager.AppSettings["ShowMultipleColors"]))
                        pen = GetPaintPenByImageNumber(item.PatternID);
                    else
                ...
                if (Convert.ToBoolean(ConfigurationManager.AppSettings["ShowMissingPatterns"]))
                {
                    coutner = 0;
                    foreach (var item in _CurrentBlobResult.ListOfInvalidBlobs)
                    {
            ...
            else if (_caputedRectangles != null && _caputedRectangles.Count > 0)
            {
                ...
                foreach (var item in _caputedRectangles)
                {
                    ...
                    System.Drawing.Rectangle srcRect = GetDisplayedImageSize(image, item);
                    Pen pen = GetPaintPenByImageNumber(coutner);
                    e.Graphics.DrawRectangle(pen, srcRect);
        ...
        private System.Drawing.Rectangle GetDisplayedImageSize(System.Drawing.Image image, SourceImageReactangle item)
        {
            int width = image.Width;
            int height = image.Height;
            ...
            float widthFactor = 0;
            float hightFactor = 0;
            var displayRectangle = this.pictureBoxCapturedImage.ClientSize;
            widthFactor = (float)width / (float)displayRectangle.Width;
            hightFactor = (float)height / (float)displayRectangle.Height;
            ...
            return new System.Drawing.Rectangle(tarX, tarY, (int)tarWidth, tarHeight);
        ...
        private void toolStripStatusSerialPortOFF_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("The Specified TCP-IP settings are invalid!");
                logger.Error(ex);
            }
            catch (Exception ex)
            {
        ...
        private void reportsToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    if (i % 2 == 0)
                        status = VisionConstants.OK_TEXT;
                    else
                        status = VisionConstants.NOTOK_TEXT;

                    data.Add(new ReportDTO() { ActualScore = i * 33, PartNo = "Part " + i.ToString(), Score = i * 3, SrNo = 1, Status = status });
                }
        ...
        private void createPatternImageToolStripMenuItem_Click(object sender, EventArgs e)
        {
            TeachImage form = new TeachImage(_currentImage);
            form.ShowDialog();
        }
        ...
        private void linkReset_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            _inspectionCounters.Reset();

            this.txtOKCounts.Text = _inspectionCounters.OKCount.ToString();
            this.txtNotOKCounts.Text = _inspectionCounters.NotOKCount.ToString();
            this.lblTotalCounts.Text = string.Format("Total Counts: ", _inspectionCounters.TotalCounts.ToString());
        }

        private void reportHistoryToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ReportsHistory form = new ReportsHistory();
            form.ShowDialog();
        }
